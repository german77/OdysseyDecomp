...
bool FileLoader::loadSoundItem(u32 itemId, u32 itemMiss, IAudioResourceLoader* loader) {
    SoundItemEntry* entry = mSoundItemHolder->tryFindEntry(itemId, loader);
    if (!entry)
        entry = requestLoadSoundItem(itemId, itemMiss, loader);
    else if (entry->getFileState() == FileState::IsLoadDone)
...
SoundItemEntry* FileLoader::requestLoadSoundItem(u32 itemId, u32 itemMiss,
                                                 IAudioResourceLoader* loader) {
    SoundItemEntry* entry = mSoundItemHolder->addNewLoadRequestEntry(itemId, itemMiss, loader);
    mLoaderThread->requestLoadFile(entry);
    return entry;
}
