...
KaronWingStateHack::KaronWingStateHack(al::LiveActor* parent, const al::ActorInitInfo& info,
    ...
    sead::Vector3f sphere{0.0f, 90.0f, 0.0f};
    mCollisionShapeKeeper->set54(35.0f);
    mCollisionShapeKeeper->createShapeSphere("Body", 70.0f, sphere);

    CollisionShapeInfoSphere* infoSphere = mCollisionShapeKeeper->getShapeInfoSphere(0);
    if (infoSphere)
        infoSphere->set48(40.0f);

    mCollisionShapeKeeper->createShapeSphereIgnoreGround("Head", 70.0, gHead);

    char* bodyParts[3] = {"LegFront", "LegLeft", "legRight"};
    for (s32 i = 0; i < 3; i++) {
        f32 angle = sead::Mathf::deg2rad(i * 120.0f) * 0.5f;
        f32 cos = sead::Mathf::cos(angle);
        f32 sin = sead::Mathf::sin(angle);
        sead::Vector3f va{cos, sin, 0.0f};
        va.rotate(sead::Quatf(0.0f, 40.0f, 0.0f, 0.0f));

        sead::Vector3f vb = -60.0f * sead::Vector3f::ey;
...
void KaronWingStateHack::attackSensor(al::HitSensor* self, al::HitSensor* other) {
    if (!al::isNerve(this, &NrvHostType.Wait) && !al::isNerve(this, &NrvHostType.Walk) &&
        al::isNerve(this, &NrvHostType.WingFly)) {
...
void KaronWingStateHack::exeWalk() {
    ...
    sead::Vector3f who = sead::Vector3f::zero;

    rs::calcHackerMoveVec(&who, *mPlayerHack, up);
    mPlayerActionTurnControl->update(who, up);
...
void KaronWingStateHack::exeEndCancel() {}

void KaronWingStateHack::exeEndReset() {}

void KaronWingStateHack::exeEndDamage() {}
